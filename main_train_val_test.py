...
def args_parser():
    ...
    parser.add_argument('--gpus', type=int, default=6, help="GPU indices ""comma separated, e.g. '0,1' ")
    ...
    parser.add_argument('--model_name',type=str, default='DSMIL', help='What model architecture to use.')
...
if __name__ == '__main__':
    ...
    # args.output_dir = os.path.join(args.output_dir, 'train_val_test_CE_loss_nSLN_bs_4', args.model_name, 'U')
    ...
    args.logger_name = 'logger'

    temp_train_csv = os.path.join(args.label_dir, 'label_for_train.csv')
    temp_valid_csv = os.path.join(args.label_dir, 'label_for_valid.csv')
    temp_test_csv = os.path.join(args.label_dir, 'label_for_test.csv')
    # train_df = pd.read_csv('/data3/yhhu/BreastCancer/非前哨淋巴结预测/label_for_train_and_valid.csv')
    # # 训练集中分出1折做验证集，用于挑选模型
    ...
    # test_df = pd.read_csv(temp_test_csv)
    # print(f'train data:{len(train_df)}, valid data:{len(valid_df)}, test data:{len(test_df)}')
    ...
    # 多折交叉验证计算结束，结果处理
    with open(os.path.join(args.output_dir, 'result.txt'), mode='w') as file:
